...
- (void)setComputeOrderBO:(ComputeOrderBO *)computeOrderBO {
    ...
    [attributedString addAttribute:NSForegroundColorAttributeName value:COMMONTOPICCOLOR range:NSMakeRange(integralRange.location,integralRange.length-2)];
    NSRange transRange = [string rangeOfString:[NSString stringWithFormat:@"%@元",computeOrderBO.totalTransportFee]];
    [attributedString addAttribute:NSForegroundColorAttributeName value:COMMONTOPICCOLOR range:NSMakeRange(transRange.location,transRange.length-1)];
