...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (_collectionViewType == FirstLevelCollectionViewType ) {//一级
        ...
        NSString *titel = dic[@"caseName"];
        CGFloat width = [titel sizeWithFont:[UIFont systemFontOfSize:14.0] maxH:10000].width;
        UIView *lineView = [cell.contentView viewWithTag:2001];
        if (lineView) {
            id hiddenObj = [_caseIsSelecedArr xf_safeObjectAtIndex:indexPath.row];
            BOOL hidden = [hiddenObj boolValue];
            lineView.hidden = !hidden;
        }else {
            UIView *line = [UIView new];
            line.width = width ;
            line.x = (5*2+5 ) / 2;
            line.y = collectionView.height -2;
            line.height = 2;
            line.backgroundColor = [YSThemeManager themeColor];
            id hiddenObj = [_caseIsSelecedArr xf_safeObjectAtIndex:indexPath.row];
            BOOL hidden = [hiddenObj boolValue];
            line.hidden = !hidden;
            line.tag = 2001;
...
//        dic[@"caseName"]
        ...
        return CGSizeMake(width+5*2+5, collectionView.height);
    }else{//橱窗商品
//        return CGSizeMake((kScreenWidth-5*4)/3, 112);
        
        return CGSizeMake((kScreenWidth-5*4)/3, kScreenWidth/(375.0/148.0));
...
{
    if (_collectionViewType == SecondLevelCollectionViewType ) {
