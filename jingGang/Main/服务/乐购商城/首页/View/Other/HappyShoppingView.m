...
@property (weak, nonatomic) IBOutlet UILabel *abbLabel;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *abbLabelHeightConstraint;
...
- (void)_addHeaderFresh {
    self.happyScrollView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        _requestFromPage = 1;
        [self _requestData];
    }];
...
//        [self _requestData];
//    }];
    self.happyScrollView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
...
//        NSLog(@"recommend %@",operation.responseObject);
        if ([adCode isEqualToString:ShoppingHomeHasYourLikeCode]) {//有您喜欢
            ...
            JGLog(@"有您细化个数 %ld",response.advList.count);
...
-(void)_requestIntegralGoodsImgWithCode:(NSString *)adCode{
        ...
        [_vapManager snsRecommendList:request success:^(AFHTTPRequestOperation *operation, SnsRecommendListResponse *response) {
            ...
            JGLog(@"积分商城首页入口图片 %@",response);
            if (response.advList.count) {
                NSDictionary *dic = response.advList[0];
                  NSString *urlStr = dic[@"adImgPath"];
                [YSImageConfig sd_view:self.integralAdImgView setimageWithURL:[NSURL URLWithString:urlStr] placeholderImage:DEFAULTIMG];
            }
...
//    }
    ...
    [_vapManager likeYouGoodsList:reqeust success:^(AFHTTPRequestOperation *operation, LikeYouGoodsListResponse *response) {
        ...
        if (_requestFromPage == 1) {//下拉刷新
            [self.happyScrollView.mj_header endRefreshing];
            self.guessYouLikeCollecionView.guessYoulikeArr = guessYouLikeArr;
        }else {//上拉加载更多
            [self.happyScrollView.mj_footer endRefreshing];
