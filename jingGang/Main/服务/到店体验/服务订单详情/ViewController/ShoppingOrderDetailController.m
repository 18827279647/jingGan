...
{
    [self showHud];
    if (self.showRefundRefuseAlert) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self hiddenHud];
            [UIAlertView xf_shoeWithTitle:@"拒退原因" message:self.refuseReasonString buttonsAndOnDismiss:@"i konw!",^(UIAlertView *alertView,NSUInteger index){
...
- (void)backToLastController {
    if (self.comeFromePushType == comeFromPayType) {
...
- (void)loadCellContent:(UITableViewCell *)cell indexPath:(NSIndexPath*)indexPath
{
    ...
    [YSImageConfig yy_view:basicCell.shopImg setImageWithURL:[NSURL URLWithString:contentDic[@"groupAccPath"]] placeholderImage:IMAGE(@"logoeee")];
...
//        [basicCell.refundBtn setEnabled:NO];
    ...
    OrderDetailDynamicCell *newCell = basicCell;
    @weakify(self);
    @weakify(newCell);
    basicCell.refundBlk = ^ {
        @strongify(self);
        @strongify(newCell);
        self.groupSn = [NSString stringWithFormat:@"%@",contentDic[@"groupSn"]];
        self.consumeState = newCell.consumeState;
        self.refundBtn = newCell.refundBtn;
...
- (void)resetDataSource:(NSDictionary *)orderDetails {
    ...
    if (orderDetails[@"storeTelephone"]) {
        self.telephone = [NSString stringWithFormat:@"%@",orderDetails[@"storeTelephone"]];
    } else {
        self.telephone = @"";
    }
    ...
    if (indexPath.section == 0) {
        ...
        cell.phoneCallBlk = ^{
            ...
            if (self.telephone.length > 0) {
                [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"telprompt://%@", self.telephone]]];
            } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        // 默认为0，使用后为1，过期为-1，审核中为3，审核通过为5，审核不通过为6，退款完成为7
        if (status.integerValue == 1 || status.integerValue == -1 || status.integerValue == 3 || status.integerValue == 5 || status.integerValue == 7) {
            return;
        }
        ...
        if(iPhoneX_X){
            ...
            qrcodeWeb.x = (ScreenWidth - 260) / 2;
            qrcodeWeb.y = 0;
            qrcodeWeb.width = 280;
            qrcodeWeb.delegate = self;
            qrcodeWeb.height =330;
            ...
            activityController.view.height = 330;
            ...
            activityController.view.backgroundColor = [UIColor clearColor];
            activityController.view.width = ScreenWidth;
            activityController.view.height = 300;
            menu.contentController = activityController;
            [menu showWithFrameWithDuration:0.25];
        }
...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (self.serviceListDataSource.count) {
            ...
            NSString *status = TNSString(detailDic[@"status"]);
            NSString *orderStatus = self.dictDetail[@"orderStatus"];
            if (status.integerValue == 0 || status.integerValue == 6) {
                
                if (orderStatus.integerValue == 80) {
                    return NO;
                }
                return YES;
            }else {
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.response) {
        // 退款申请
        // self.serviceListDataSource
        ApplySalesReturnVC *vc = [[ApplySalesReturnVC alloc] initWithResponse:self.response];
        NSDictionary *serviceDataSource = [NSDictionary dictionaryWithDictionary:(NSDictionary *)[self.serviceListDataSource xf_safeObjectAtIndex:indexPath.row]];
        vc.groupSn = [NSString stringWithFormat:@"%@",serviceDataSource[@"groupSn"]];
