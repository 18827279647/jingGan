...
-(void)_loadTitleView{
    ...
    [YSThemeManager setNavigationTitle:@"提交订单" andViewController:self];
}
...
//    submit.backgroundColor = [YSThemeManager buttonBgColor];
    submit.layer.cornerRadius = kRadius;
    submit.layer.masksToBounds = YES;
    [submit setTitle:@"提交订单" forState:UIControlStateNormal];
    [submit setBackgroundImage:[UIImage imageNamed:@"button222"] forState:UIControlStateNormal];
    [submit setBackgroundImage:[UIImage imageNamed:@"button222"] forState:UIControlStateSelected];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section < self.dataSource.count) {
        ...
        if (indexPath.section == 0) {
            ...
            NSArray *imgArr = [self.serviceDetail.groupAccPath componentsSeparatedByString:@";"];
            
            NSString *strImageUrl = [imgArr xf_safeObjectAtIndex:0];
            
            [YSImageConfig yy_view:groupImage setImageWithURL:[NSURL URLWithString:strImageUrl] placeholderImage:DEFAULTIMG];
          ...
          AddSubCountView *addSubCountView = [AddSubCountView showInContentView:cell goodStockCount:self.maxNum size:CGSizeMake(160, 32)];
            @weakify(self);
            addSubCountView.countChangedBlk = ^(NSInteger count){
                @strongify(self);
                self.totalCountLb.text = [NSString stringWithFormat:@"%.2f", [self.price floatValue] * count];
            };
          self.addSubCountView = addSubCountView;
            ...
            cell.detailTextLabel.textColor = COMMONTOPICCOLOR;
