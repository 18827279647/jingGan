...
@property (nonatomic, assign) SGTiTleType TiTleType;
@end

@implementation SGPageTitleView

- (instancetype)initWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames configure:(SGPageTitleViewConfigure *)configure SGTiTleType:(SGTiTleType)TiTleType{
...
+ (instancetype)pageTitleViewWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames configure:(SGPageTitleViewConfigure *)configure SGTiTleType:(SGTiTleType)TiTleType{
    return [[self alloc] initWithFrame:frame delegate:delegate titleNames:titleNames configure:configure SGTiTleType:(SGTiTleType)TiTleType];
}
...
- (void)setupTitleButtons {
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { // SGPageTitleView 静止样式
        ...
        for (NSInteger index = 0; index < titleCount; index++) {
            ...
            if (self.TiTleType == SGTitleNewline) {
                btn.titleLabel.numberOfLines = 0;
                NSString * titleString = self.titleArr[index];
                ...
                if (titleString.length > 4) {
                    ...
                    [String addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:10] range:NSMakeRange(5,titleString.length - 5)];
                    ...
                    [btn.titleLabel setAttributedText:String];
                }else {
...
- (void)P_changeSelectedButton:(UIButton *)button {
    if (self.tempBtn == nil) {
        ...
        if (self.TiTleType == SGTitleNewline) {
            NSString * temptitleString = self.tempBtn.titleLabel.text;
            NSMutableAttributedString * tempString = [[NSMutableAttributedString alloc]initWithString:temptitleString];
            if (temptitleString.length > 4) {
                [tempString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:10] range:NSMakeRange(5,temptitleString.length - 5)];
                [tempString addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"959595"] range:NSMakeRange(5,temptitleString.length - 5)];
            }
            
            [self.tempBtn.titleLabel setAttributedText:tempString];
                ...
                if (titleString.length > 4) {
                    [String addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:10] range:NSMakeRange(5,titleString.length - 5)];
                    [String addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"65BBB1"] range:NSMakeRange(5,titleString.length - 5)];
                }
