...
#define KButtonTitleToleftRightEdge 15

//默认按钮颜色
#define kDefaultColor [UIColor clearColor];
//默认按钮选中颜色
#define kDefaultSelectedColor [UIColor clearColor];

//屏幕宽度
#define WkScreenWidth [[UIScreen mainScreen] bounds].size.width - 10
...
-(void)_configureButton:(UIButton *)textButton{
    ...
    //按钮默认与没选中的字体与边框颜色是UIColorFromRGB(0x979797)
    ...
    textButton.layer.borderColor = UIColorFromRGB(0x979797).CGColor;
    textButton.layer.borderWidth = 1;
    [textButton setTitleColor:UIColorFromRGB(0x979797) forState:UIControlStateNormal];
}
...
-(void)setButtonStateColorOfButton:(UIButton *)button{
    ...
    if (button.selected) {
        button.layer.borderColor = kGetColor(107,187,177).CGColor;
        [button setTitleColor:kGetColor(107,187,177) forState:UIControlStateNormal];
    }else{
        button.layer.borderColor = UIColorFromRGB(0x979797).CGColor;
