...
@interface RXWmViewController ()<WKUIDelegate,WKNavigationDelegate,UIScrollViewDelegate>
...
@implementation RXWmViewController
...
- (void)webView:(WKWebView *)webView runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(void))completionHandler {
    if (message) {
        if ([message rangeOfString:@"lookArchivesDetails"].location != NSNotFound) {
            NSString *str4 = [message substringFromIndex:19];
            RXUserH5UrlRequest*request=[[RXUserH5UrlRequest alloc]init:GetToken];
            VApiManager *manager = [[VApiManager alloc]init];
            [manager RXRXUserH5UrlRequest:request success:^(AFHTTPRequestOperation *operation, RXUserH5UrlResponse *response) {
                if (response.isMember!=1) {
                    [self showStringHUD:@"会员到期" second:0];
                }else{
                    VApiManager *manager = [[VApiManager alloc]init];
                    RXUserOtherRequest*request=[[RXUserOtherRequest alloc]init:GetToken];
                    request.id=str4;
                    [manager RXUserOtherRequest:request success:^(AFHTTPRequestOperation *operation, RXUserOtherResponse *response) {
                        NSString*string=@"完美档案报告";
                        RXWmViewController*view=[[RXWmViewController alloc]init];
                        view.titlestring=string;
                        view.htmlstring=response.message;
...
- (void)webView:(WKWebView *)webView runJavaScriptTextInputPanelWithPrompt:(NSString *)prompt defaultText:(NSString *)defaultText initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(NSString * _Nullable))completionHandler {
    if (prompt) {
        if ([prompt isEqualToString:@"getType"]) {
