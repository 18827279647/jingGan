...
//    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"home_title"] forBarMetrics:UIBarMetricsDefault];
    ...
    [YSThemeManager setNavigationTitle:@"历史纪录" andViewController:self];
...
-(void)_requestWeekStepData{
    ...
    request.api_startDateStr = [self getDateInSubtractionDays:7];
    request.api_endDateStr = [self getDateInSubtractionDays:1];
    
    
    WeakSelf;
    [_vapManager equipQueryByRange:request success:^(AFHTTPRequestOperation *operation, EquipQueryByRangeResponse *response) {
        StrongSelf;
        strongSelf.totalStep = [response.totalStepNumber integerValue];
        strongSelf.totalKm = [response.totalDistance integerValue];
        strongSelf.totalKaluoli = [response.totalCalories integerValue];
...
#pragma mark --返回当前天减去第几天
- (NSString *)getDateInSubtractionDays:(NSInteger)subtractionDays
{
    //得到当前的时间
    ...
    NSTimeInterval time = 60 * 60 * 24 * subtractionDays;//一周的秒数
