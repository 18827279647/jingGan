...
-(JGShareView *)shareView {
    ...
    if ([self.img_name isEqualToString:@"home_sleep"]) {//睡眠控制器
        shareTitle = @"邀你一起动起来~";
        shareContent = [NSString stringWithFormat:@"+昨晚睡了%@，深睡%@，浅睡%@。e生康缘手环，帮你睡得还好.",totalTime,deepTime,slightTime];
    }else{//运动步数控制器
        shareTitle = @"它让你睡得更好了~";
        shareContent = [NSString stringWithFormat:@"+%ld月%ld日，我一共行走了%.1f公里，%ld步，消耗热量%.1f卡路里，e生康缘手环，健康运动！",month,day,_jgBlueManager.userBodySyncModel.licheng,_jgBlueManager.userBodySyncModel.steps,_jgBlueManager.userBodySyncModel.kaluoli];
