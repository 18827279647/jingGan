...
//    NSInteger i = _img_choose_array.count-1;
        ...
        [YSImageConfig yy_view:little_img setImageWithURL:[NSURL URLWithString:img_url] placeholderImage:nil];
...
-(void)upLoadHeadImgWithImgData:(NSData *)imgData{
    ...
    [manager POST:postImageUrl parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if (imageItems.count) {
            NSString *newHeadImgUrl =  [[responseObject[@"items"] xf_safeObjectAtIndex:0] objectForKey:@"fileUrl"];
            if ([self.bar_title isEqualToString:@"修改提问"]) {
