...
- (void)viewDidLoad {
    ...
    [YSThemeManager setNavigationTitle:@"健康咨询" andViewController:self];
    ...
    @weakify(self)
    _left_tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        @strongify(self)
        pageSize = pageSize+1;
...
//    }];
    ...
    _right_tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        pageSizeRight += 1;
        @strongify(self)
...
//        [_myScrollView footerEndRefreshing];
        if (array.count == 0) {
            ...
            if (number == 1) {
                [_left_tableView.mj_footer endRefreshing];
            }else{
                [_right_tableView.mj_footer endRefreshing];
            }
        }
...
- (TLTitleSelectorView *)titleView {
    if (_titleView == nil) {
        ...
        _titleView.selectedColor = COMMONTOPICCOLOR;
        @weakify(self)
