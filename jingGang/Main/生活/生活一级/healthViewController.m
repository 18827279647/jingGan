...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    // 友盟统计
    [YSUMMobClickManager endLogPageWithKey:kLifeViewController];
...
#pragma mark - 修改
    [YSThemeManager setNavigationTitle:@"健康生活" andViewController:self];
...
-(void)_requestCareChoosenGoods{
    ...
    [vapManager snsCrcList:request success:^(AFHTTPRequestOperation *operation, SnsCrcListResponse *response) {
        ...
        JGLog(@"生活首页精选商品 response %@",response);
...
-(void)_requestGoodStoreRecommend {
    ...
    [vapManager snsCrcList:request success:^(AFHTTPRequestOperation *operation, SnsCrcListResponse *response) {
        ...
        JGLog(@"生活首页好店推荐 response %@",response);
        NSInteger itemCount = response.advList.count;
        if (itemCount > 0) {
    ...
    if (indexPath.section == 0) {
        if (indexPath.row < arr_data.count) {
            if (arr_data.count >0) {
                ...
                [YSImageConfig sd_view:cell.left_img setimageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"background"]];
