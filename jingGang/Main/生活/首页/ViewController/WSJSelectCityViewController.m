...
//    };
    [YSThemeManager setNavigationTitle:[NSString stringWithFormat:@"当前城市-%@",    [[mapObject sharedMap] baiduCity]] andViewController:self];
    ...
    self.tableHeaderView.headerAction = ^(NSString *city,NSNumber *cityID){
        ...
        if (self.city)
        {
            self.map.baiduCity = city;
            self.map.baiduCityID = cityID;
            self.city(city,cityID);
            NSUserDefaults *defaults = kUserDefaults;
