...
@interface UsersInfo ()<UITextFieldDelegate, UIScrollViewDelegate,UIActionSheetDelegate>
{
...
@property (strong,nonatomic) UIButton *btnClicks;
...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        dictss = [[NSUserDefaults standardUserDefaults]objectForKey:kUserCustomerKey];
...
-(void)bloodSelect
{
    ...
    self.bloodView = [[UIView alloc]initWithFrame:CGRectMake(self.frame.size.width-50, 440, 40, 120)];
    [_scroll_other addSubview:self.bloodView];
    CGFloat widthss = 40;
    CGFloat heightss = 30;
    for (int i = 0; i<4; i++) {
       ...
       // btn.backgroundColor =[UIColor yellowColor];
        ...
        [self.bloodView addSubview:btn];
    }
   //bloodView.hidden = NO;
    self.bloodView.backgroundColor = [UIColor whiteColor];
}
...
-(void)loadCalenderView{
    ...
    _calenderView.selectBirthBlock = ^(NSString *year,NSString *month,NSString *day){
        ...
        [self.btnClicks setTitle:strBirth forState:UIControlStateNormal];
...
//    [name_text addTarget:self action:@selector(textEditing:) forControlEvents:UIControlEventEditingChanged];
    ...
    self.btnClicks = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(data_l.frame) +3, 0, 150, 42)];
    self.btnClicks.titleLabel.textAlignment = NSTextAlignmentLeft;
    self.btnClicks.titleLabel.font = [UIFont systemFontOfSize:14];
    [self.btnClicks setTitleColor:[[UIColor blackColor] colorWithAlphaComponent:0.7] forState:UIControlStateNormal];
    [self.btnClicks setTitle:[[NSUserDefaults standardUserDefaults]objectForKey:@"birthdate"] forState:UIControlStateNormal];
    self.birthDayStr = self.btnClicks.titleLabel.text;
    
    [self.btnClicks addTarget:self action:@selector(BtnCLicksss) forControlEvents:UIControlEventTouchUpInside];
    
    [data_view addSubview:data_l];
   
   [data_view addSubview:self.btnClicks];
