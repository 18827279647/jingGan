...
- (void)certainAlter
{
    ...
    [_vapiManager usersPayPasswordUpdate:passwordRequest success:^(AFHTTPRequestOperation *operation, UsersPayPasswordUpdateResponse *response) {
        if (!response.errorCode) {
            [weak_self showAlertMessage:@"修改健康豆密码成功"];
        }
        else
        {
            [weak_self showAlertMessage:@"修改健康豆密码失败"];
...
- (void) createUI
{
    ...
    NSDictionary *dict = [[NSUserDefaults standardUserDefaults]objectForKey:kUserCustomerKey];
    ...
    self.yanzhengBtn.backgroundColor = [YSThemeManager buttonBgColor];
...
- (void) yanzhengAction:(UIButton *)btn
{
    if (_time == -1)
    {
        ...
        YSDynamicVerifyController *viewCtrl = [[YSDynamicVerifyController alloc] initWithTelephoneNumber:_phone];
