...
- (void)initScrollView
{
    ...
    for (int i = 0; i < self.arrayStarImage.count; i ++)
    {
        ...
        if (i == self.arrayStarImage.count - 1) {
            ...
            if (iPhone4||iPhone5) {
                buttonRect = CGRectMake(0, kScreenHeight - 60-50 , 120, 50);
            }else if (iPhone6){
                buttonRect = CGRectMake(0, kScreenHeight - 69-60, 160, 70);
            }else if(iPhoneX_X){
                 buttonRect = CGRectMake(0, 0 , self.view.frame.size.width, self.view.frame.size.height);
            }else
            {
                buttonRect = CGRectMake(0, kScreenHeight - 75-70 , 200, 85);
            }
...
//            [self.startButton setTitleColor:[YSThemeManager buttonBgColor] forState:UIControlStateNormal];
...
//                [self.startButton.layer setBorderWidth:1];//设置边界的宽度
                //设置按钮的边界颜色
                CGColorRef color = [YSThemeManager buttonBgColor].CGColor;
                [self.startButton.layer setBorderColor:color];
                [self.startButton setBackgroundColor:[UIColor clearColor]];
            }else {
                self.startButton.y += 10.;
                [self.startButton setBackgroundColor:JGColor(220, 42, 58, 1)];
            }
...
//    AppDelegate * app = (AppDelegate*)[[UIApplication sharedApplication]delegate];
//    [app beGinLoginWithType:YSLoginLeaderCloseType toLogin:NO];
    AppDelegate * app = (AppDelegate*)[[UIApplication sharedApplication]delegate];
    [app gogogoWithTag:0 shouldNotityMainController:NO];
}
