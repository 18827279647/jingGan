...
- (void)viewDidLoad {
    ...
    self.strTitle = [NSString stringWithFormat:@"%@年%@月%@日体检报告",self.strYear,self.strMonth,self.strDay];
...
- (void)netWorkRequest
{
    ...
    [self.netManager reportResultDetails:request success:^(AFHTTPRequestOperation *operation, ReportResultDetailsResponse *response){
        ...
        [bself.scrollViewBg.mj_header endRefreshing];
        [bself.scrollViewBg.mj_footer endRefreshing];
...
{
    if (isSetFrame) {
        self.buttonSave.frame = CGRectMake(25, 69, self.tableView.frame.size.width - 50, 42);
        ...
        self.buttonSave.frame = CGRectMake(buttonSaveX, 69, self.buttonCheckReport.frame.size.width, 42);
...
- (void)checkReportbuttonCilck
{
    ...
    [YSThemeManager setNavigationTitle:self.strTitle andViewController:vc];
...
{
    if (!_tableView) {
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 10, __MainScreen_Width, __MainScreen_Height - 44) style:UITableViewStylePlain];
...
- (UIButton *)buttonSave
{
    if (!_buttonSave) {
        ...
        _buttonSave.frame = CGRectMake(35, 69, kScreenWidth - 70, 44);
//        _buttonSave.backgroundColor = [YSThemeManager buttonBgColor];
        ...
        [_buttonSave setBackgroundImage:[UIImage imageNamed:@"button222"] forState:UIControlStateNormal];
...
{
    UIView *viewBg = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 143)];
    viewBg.backgroundColor = UIColorFromRGB(0xf7f7f7);
    UIView *topWhiteView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 39)];
    topWhiteView.backgroundColor = [UIColor whiteColor];
    UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, topWhiteView.height - 0.5, kScreenWidth, 0.5)];
    lineView.backgroundColor = UIColorFromRGB(0xdddddd);
    [topWhiteView addSubview:lineView];
    
    [viewBg addSubview:topWhiteView];
    [viewBg addSubview:self.buttonSave];
    [viewBg addSubview:self.buttonCheckReport];
...
- (UIScrollView *)scrollViewBg
{
    if (!_scrollViewBg) {
        ...
        _scrollViewBg.backgroundColor = [UIColor whiteColor];
        [_scrollViewBg addSubview:self.tableView];
        
        typeof (self) bself = self;
        self.scrollViewBg.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
            [bself tableViewHeaderRereshing];
        }];
        self.scrollViewBg.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
...
- (UIButton *)buttonCheckReport
{
    if (!_buttonCheckReport) {
        ...
        _buttonCheckReport.frame = CGRectMake(5, self.buttonSave.y, (self.tableView.frame.size.width - 10 - 10)/2, 42);
