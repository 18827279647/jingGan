...
@property (weak, nonatomic) IBOutlet UILabel *cashTitel;
...
- (void)configContent {
    ...
    self.cashTitel.textColor = [YSThemeManager buttonBgColor];
//    self.segmentedControl.tintColor = [YSThemeManager buttonBgColor];
...
//    [completeButton setBackgroundColor:[YSThemeManager buttonBgColor]];
    [completeButton setBackgroundImage:[UIImage imageNamed:@"button222"] forState:UIControlStateNormal];
...
- (BOOL)verifyUserCommitInfo:(void(^)(NSInteger selectedPayType,id infoModel))info {
    if (self.segmentedControl.selectedSegmentIndex == 0) {
        // 验证银行卡
        if (self.bankView.userNameTextField.text.length == 0 ||  self.bankView.openAccountBankTextField.text.length == 0  || self.bankView.branchBankTextField.text.length == 0 ) {
            // 信息不全
            ...
            if (self.bankView.bankCardNumberTextField.text.length < 16 ||self.bankView.bankCardNumberTextField.text.length > 20) {
        ...
        if (self.aliView.userNameTextField.text.length == 0 || self.aliView.aliAccountTextField.text.length == 0 ) {
            return NO;
        }else if([Util isValidateMobile:self.aliView.aliAccountTextField.text] || [Util isValidateEmail:self.aliView.aliAccountTextField.text]) {
            ...
            [UIAlertView xf_showWithTitle:@"支付宝账号信息有误" message:nil delay:1.2 onDismiss:^{
...
- (IBAction)selectedCashTypeAction:(UISegmentedControl *)sender {
    switch (sender.selectedSegmentIndex) {
        case 0:
        {
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"为了确保及时到账,请优先使用四大行" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
            [alertView show];
            // 银行卡提现
            self.contentScrollView.contentOffset = CGPointMake(0, 0);
        }
