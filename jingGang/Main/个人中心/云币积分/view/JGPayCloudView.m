...
- (void)setup {
    [self.wxPayButton setImage:[[UIImage imageNamed:@"zhifuwei"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
    //DUI-DE
    [self.wxPayButton setImage:[[UIImage imageNamed:@"zhifuxuan"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateSelected];
    self.wxPayButton.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0,ImageEdgeInsetsTop);
    self.wxPayButton.selected = YES;
    
    
    [self.alPayButton setImage:[[UIImage imageNamed:@"zhifuwei"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
    [self.alPayButton setImage:[[UIImage imageNamed:@"zhifuxuan"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateSelected];
    self.alPayButton.imageEdgeInsets = self.wxPayButton.imageEdgeInsets;
    
 
//    self.payCashButton.backgroundColor = [YSThemeManager buttonBgColor];
}
...
- (void)queryDealResult:(BOOL)success response:(id)response {
    if (success) {
        ...
        [WSProgressHUD dismiss];
        [KJShoppingAlertView showAlertTitle:@"健康豆充值失败" inContentView:self];
    }
}
