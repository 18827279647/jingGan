...
- (void)setupContentInterface {
    ...
    switch (self.successType) {
        ...
        {
           _image = [UIImage imageNamed:@"jg_cashDetail"];
            _imageWidth = _image.imageWidth;
           _imageHeight = _image.imageHeight;
            
        }
            break;
        case PaySuccessInterfaceType:
        {
           _image = [UIImage imageNamed:@"jg_paysuccess"];
            _imageWidth = _image.imageWidth;
           _imageHeight = _image.imageHeight;
   ...
   succImage.image = _image;

    succImage.x = (ScreenWidth - _imageWidth) / 2;
    succImage.y = 90.0 / 2 ;
    succImage.width = _imageWidth;
    succImage.height = _imageHeight;
    ...
    textLab.font = JGFont(20);
    textLab.textColor =  JGColor(93,187,177,1);
    ...
    switch (self.successType) {
        ...
        {
            // 提现界面初始化
            [YSThemeManager setNavigationTitle:@"提现详情" andViewController:self];
//            succImage.image = [UIImage imageNamed:@"jg_cashDetail"];
        ...
        {
            // 充值界面初始化
            [YSThemeManager setNavigationTitle:@"充值详情" andViewController:self];
//            succImage.image = [UIImage imageNamed:@"jg_paysuccess"];
    ...
    UILabel *payValueLab = [self createLabWithFrame:CGRectMake(payTimeLab.x, CGRectGetMaxY(payTimeLab.frame) + labMarginY, payTimeLab.width, payTimeLab.height) backgroundColor:[UIColor clearColor] text:[NSString stringWithFormat:@"￥%.2f",[actionAmount floatValue]] font:JGFont(13) textColor: JGColor(93,187,177,1) alignment:NSTextAlignmentRight];
