...
+ (void)inquiryCashInformationResponseSuccess:(id_block_t)success fail:(msg_block_t)fail  {
    VApiManager *manager = [[VApiManager alloc] init];
    MoneyFreePoundageRequest *request = [[MoneyFreePoundageRequest alloc] init:GetToken];
    [manager moneyFreePoundage:request success:^(AFHTTPRequestOperation *operation, MoneyFreePoundageResponse *response) {
 ...
 *  @param cashId        健康豆提现id  有默认cashid 就传cashid 没有或重新新选择另外一个支付方式或银行就传空
 */
