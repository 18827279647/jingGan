...
- (void)viewDidLoad {
    ...
    _textfied.borderStyle = UITextBorderStyleBezel;
...
- (void)loadUI
{
    [YSThemeManager setNavigationTitle:@"填写邀请码" andViewController:self];
    //设置textField的边框
    self.textFieldInviteCode.layer.borderWidth = 1.5f;
    self.textFieldInviteCode.layer.cornerRadius = 25;
    self.textFieldInviteCode.layer.borderColor = rgb(101, 187, 177, 1.0).CGColor;
//    self.textFieldInviteCode.textAlignment = NSTextAlignmentCenter;
    
    //验证按钮设置圆角
    self.buttonInviteCode.layer.cornerRadius = 6.0;
//    self.buttonInviteCode.backgroundColor = [YSThemeManager buttonBgColor];
}

- (IBAction)buttonTestCodeClick:(id)sender {
    [self.view endEditing:YES];
    if ([Util varifyValidOfStr:self.textFieldInviteCode.text]) {
        if (self.textFieldInviteCode.text.length < 7 || self.textFieldInviteCode.text.length > 7) {
            [self hideHubWithOnlyText:@"邀请码应该是7位数字"];
            return;
        }
        //如果格式没有错误就提交验证码请求
        [MBProgressHUD showHUDAddedTo:self.view animated:YES];
