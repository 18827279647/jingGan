...
- (void) initUI
{
    ...
    self.titleLabel.textColor = JGColor(166, 166, 166, 1);
    ...
    self.dateLabel = [[UILabel alloc] initWithFrame:CGRectMake(__MainScreen_Width-150, 10, __MainScreen_Width - 170, CGRectGetHeight(self.titleLabel.frame))];
...
- (void)setModel:(WSJEvaluateModel *)model
{
    ...
    _model = model;
    _saveModel = model;
    [YSImageConfig yy_view:self.titleImageView setImageWithURL:[NSURL URLWithString:model.titleImageURL] placeholderImage:[UIImage imageNamed:@"home_head"]];
...
#pragma mark -  //附加图片
    ...
    if (model.dataImageArray.count > 0)
    {
        ...
        [model.dataImageArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            [YSImageConfig yy_view:img setImageWithURL:[NSURL URLWithString:obj] placeholderImage:DEFAULTIMG];
            img.tag = idx;
