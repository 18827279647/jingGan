...
- (void) setWithShopCount:(NSInteger)count withIntegral:(NSInteger)integral withPostal:(CGFloat )postal
{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:16],NSFontAttributeName,COMMONTOPICCOLOR,NSForegroundColorAttributeName, nil];
...
- (void)setRightBtnStatus:(kOrderStatus)status withButton:(UIButton *)btn
{
    ...
    switch (status) {
        ...
        case kCertain://确定收货
        {
            btn.backgroundColor = [YSThemeManager buttonBgColor];
