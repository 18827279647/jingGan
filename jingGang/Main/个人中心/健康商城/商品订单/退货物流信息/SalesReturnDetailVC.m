...
- (void)setUIContent {
    ...
    [YSThemeManager setNavigationTitle:@"退货物流信息" andViewController:self];
...
- (UILabel *)supplier {
    if (_supplier == nil) {
        ...
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:@"本次退货服务由 e生康缘 为您提供"];
        [attributedString setAttributes:@{NSForegroundColorAttributeName : labelTextColor} range:NSMakeRange(0, attributedString.length)];
        NSRange range = [attributedString.string rangeOfString:@"e生康缘"];
        [attributedString setAttributes:@{NSForegroundColorAttributeName : COMMONTOPICCOLOR} range:range];
