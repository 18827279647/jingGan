...
//            self.groupSn = TNSString(serviceListDic[@"groupSn"]);
        ...
        [YSThemeManager setNavigationTitle:@"退款申请" andViewController:self];
        self.titleDescription.text = @"退款申请通过后,款项将以健康豆的形式返回至您的账户";
...
-(void)_requestOrderRefund:(NSString *)api_refund{
    ...
    [manager personalCouponRefund:request success:^(AFHTTPRequestOperation *operation, PersonalCouponRefundResponse *response) {
        if (IsEmpty(response.subCode)) {
            ...
            [UIAlertView xf_showWithTitle:@"提示"
                                  message:@"您的退款申请正在受理，请耐心等候!"
                                    delay:1.0 onDismiss:^{
...
- (void)setUIContent {
    ...
    [YSThemeManager setNavigationTitle:@"申请退换货" andViewController:self];
    self.applyText.returnKeyType = UIReturnKeyNext;
...
{
    if (self.response) {
        [YSImageConfig yy_view:self.goodLogo setImageWithURL:[NSURL URLWithString:photoPath] placeholderImage:DEFAULTIMG];
    }else {
        [YSImageConfig yy_view:self.goodLogo setImageWithURL:[NSURL URLWithString:photoPath] placeholderImage:DEFAULTIMG];
    }
}
