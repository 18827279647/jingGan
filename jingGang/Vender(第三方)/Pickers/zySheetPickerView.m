...
@property (assign,nonatomic)NSIndexPath *indexPath;//回调选择哪一行
...
-(void)setupUI
{
    ...
    //  标题
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_SIZE.width/2-75, 12, 150, 20)];
    titleLabel.font = JGFont(18);
    ...
    doneButton.frame = CGRectMake(KScreenWidth-KScreenWidth*0.2-2, 6, KScreenWidth*0.2, 26);
...
- (void)clicked:(UIButton *)sender
{
    if ([sender isEqual:self.cancelButton]) {
        ...
        if (self.callBack) {
            self.callBack(self,_backString,_indexPath);
        }
    }
}
...
- (void)show
{
    [UIView animateWithDuration:0.5f delay:0 usingSpringWithDamping:0.8f initialSpringVelocity:1.0f options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionLayoutSubviews animations:^{
