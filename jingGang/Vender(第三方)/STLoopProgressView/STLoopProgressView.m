...
- (void)setupColorLayer {
    ...
    // 分段设置渐变色
    leftLayer.locations = @[@0.3, @0.99];
    leftLayer.colors = @[(id)[STLoopProgressView centerColorone].CGColor, (id)[STLoopProgressView startColorone].CGColor];
    [self.colorLayer addSublayer:leftLayer];
    
    CAGradientLayer *rightLayer = [CAGradientLayer layer];
    rightLayer.frame = CGRectMake(SELF_WIDTH / 2, 0, SELF_WIDTH / 2, SELF_HEIGHT);
    rightLayer.locations = @[@0.3, @0.99];
    rightLayer.colors = @[(id)[STLoopProgressView centerColorone].CGColor, (id)[STLoopProgressView endColorone].CGColor];
    [self.colorLayer addSublayer:rightLayer];
    
    CAGradientLayer *colorLayer11 = [CAGradientLayer layer];
    colorLayer11.frame = CGRectMake(SELF_WIDTH / 2, 0, SELF_WIDTH / 2, SELF_HEIGHT);
   [self.colorLayer addSublayer: colorLayer11];


    
//    CAGradientLayer *rightLayerone = [CAGradientLayer layer];
//    rightLayerone.frame = CGRectMake(0, 0, SELF_WIDTH, SELF_HEIGHT);
//    rightLayerone.locations = @[@0.3, @0.9, @1];
//    rightLayerone.colors = @[(id)[STLoopProgressView centerColor].CGColor, (id)[STLoopProgressView endColor].CGColor];
//    [self.colorLayer addSublayer:rightLayerone];
}
